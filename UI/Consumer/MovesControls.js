...
export default class MovesControls extends Component {
  state = {
    index: LoginData.CurrentPage,Badge:'0',
    routes: [
      { key: 'Moves', title: 'My Quotes',icon:'file-document-edit',type:'material-community' },
      { key: 'Active', title: 'Active Moves',icon:'text-box-check',type:'material-community' },
      { key: 'Previus', title: 'Previous Moves',icon:'text-box-search',type:'material-community' },
...
componentWillUnmount() {
 ...
 .catch( (error)=> {
  ...
  _renderScene = ({ route, jumpTo }) => {
    switch (route.key) {
   case 'Moves':
    return <MyQoutes navigation={this.props.navigation}  jumpTo={jumpTo} />;
    case 'Active':
      return <ActiveMove navigation={this.props.navigation}  jumpTo={jumpTo} />;
      case 'Previus':
        return <PreviousMoves navigation={this.props.navigation}  jumpTo={jumpTo} />;
        case 'Notif':
